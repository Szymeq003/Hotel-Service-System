...
class FrontendController extends Controller
{
    ...
    public function roomsearch(Request $request)
    {
        ...
        {
            if (!$request->ajax())
            return redirect('/')->with('norooms', __('Nie znaleziono ofert spełniających kryteria'));
        }
    ...
    public function makeReservation($room_id, $city_id, Request $request)
    {
        ...
        if(!$avaiable)
        {
            if (!$request->ajax())
            {
                $request->session()->flash('reservationMsg', __('Brak możliwości rezerwacji w tym terminie'));
